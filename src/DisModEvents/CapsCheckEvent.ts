...
export default class CapsCheckEvent extends DisModBaseEvent {
      constructor() {
            super('capsCheck');
      }

      async run(client: DiscordClient, message: Message, capsAmount: number) {
            ...
            if (
                  capsAmount <
                  (config as GuildConfig).autoModerator.antiCapsCheck.capsAmount
            )
                  return;

            if (config?.autoModerator.antiCapsCheck.enabled) {
                  if (config?.autoModerator.antiCapsCheck.warnMember) {
                  ...
                  if (config?.autoModerator.antiCapsCheck.deleteMessage) {
