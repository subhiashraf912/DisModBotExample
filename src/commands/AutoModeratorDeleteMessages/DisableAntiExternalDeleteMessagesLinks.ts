...
export default class EnableAntiEmojiSpamCommand extends BaseCommand {
      constructor() {
            super({
                  name: 'disable-anti-external-links-message-delete',
      ...
      async run(client: DiscordClient, message: Message, args: Array<string>) {
            ...
            if (autoModerator.antiExternalLink.deleteMessage === false)
                  return message.channel.send(
                        'lazy to type :v replace this with your message',
                  );
            autoModerator.antiExternalLink.deleteMessage = false;
